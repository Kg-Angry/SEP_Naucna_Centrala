    ...
    if(this.korisnik.korpa.naucni_casopis_list.length === 0)
    {
                ...
                for(let j=0; j < this.zajednickiTipovi.length; j++)
                {
                  for(let i = 0; i < this.tipPlacanjaCasopisa[k].tipoviPlacanja.length; i++){
                      if(this.zajednickiTipovi[j] === this.tipPlacanjaCasopisa[k].tipoviPlacanja[i])
                      {
                        ...
                        if(!casopisDodat){
                          if(this.korisnik.korpa.naucni_rad_list.length > 0)
                          {
                            let moze=false;
                            for(let c = 0; c < this.korisnik.korpa.naucni_rad_list.length; c++){
                                if(rad.naucni_casopis.naziv === this.korisnik.korpa.naucni_rad_list[c].naucni_casopis.naziv)
                                {
                                    moze=true;
                                }
                            }
      ...
      if(dodaoUkorpu === 0){
        Swal.fire({
          position: 'top-end',
          icon: 'error',
          title: 'Nije moguce dodati rad u korpu',
